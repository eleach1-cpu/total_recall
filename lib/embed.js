...
const embedModel = (cfg) => cfg.embed?.model || (require('./voyage').provider(cfg) === 'voyage' ? 'voyage-4-lite' : 'nomic-embed-text');
function localOnly(cfg) {
  if (require('./voyage').provider(cfg) !== 'ollama') throw new Error('Legacy embed is local-only. Voyage uses the full-text index with explicit remote-upload permission and a per-run limit.');
}
