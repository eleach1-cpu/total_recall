...
function decorate(store, d) {
  d = { ...d, body: require('./decide').displayBody(d) };
...
async function find(store, a, continuation, ctx) {
  ...
  if (o.match === 'substring' && o.expr) {
    ...
    try {
      // Keep FTS outermost: a docs-first plan repeats the full-text MATCH for every record.
      rows = match ? store.db.prepare(`SELECT ${META},bm25(docs_fts) rank FROM docs_fts CROSS JOIN docs d ON d.id=docs_fts.rowid WHERE docs_fts MATCH ? AND ${f.where} ORDER BY rank,d.id`).all(match, ...f.params)
...
function inventory(store, a, continuation) {
  ...
  const ingest = require('./ingest'), configSha = ingest.coverageConfigSha(store.cfg);
  const receipt = (completed) => {
    let r; try { r = JSON.parse(store.db.prepare('SELECT value FROM meta WHERE key=?').get(ingest.receiptKey(store.cfg, completed))?.value); } catch {}
    return r?.project === store.cfg.project && r.config_sha === configSha ? r : null;
  };
  const attempt = receipt(false), completed = receipt(true);
  return { ...base(store), imported: span, clients, configured_sources: store.cfg.transcriptSources || [], checkpoints: checkpoints.slice(offset, offset + limit), source_checkpoint_count: checkpoints.length,
    ...
    configured_note_sources: store.cfg.sources || {}, last_ingest_attempt: attempt,
    last_completed_ingest: completed?.finished_at || null, last_completed_ingest_receipt: completed,
    unregistered_or_missing_sources: null, outside_corpus: 'Only configured imported Claude Code/Codex local records and note files. This is not all ChatGPT web project history.',
    meaning: require('./recall-meaning').coverage(store),
    notes: ['Unknown is null, not zero. Source checkpoints do not prove a complete ingest. A completion receipt describes its configured file snapshots, not later appends or unconfigured sources. Completed can carry warnings; inspect coverage_complete and the latest attempt. No directories were scanned and no ingest or embedding job was started.'] };
}
...
function format(r) {
  ...
  for (const k of ['projects','sessions','imported','clients','configured_sources','configured_note_sources','checkpoints','last_source_checkpoint','last_completed_ingest','last_ingest_attempt','last_completed_ingest_receipt','unregistered_or_missing_sources','outside_corpus']) if (r[k] !== undefined) lines.push(`${k}: ${JSON.stringify(r[k])}`);
