...
function walkJsonl(dir, recursive, out = []) {
  let entries = [];
  try { entries = fs.readdirSync(dir, { withFileTypes: true }); } catch { return out; }
  for (const e of entries) {
    const p = path.join(dir, e.name);
    if (e.isDirectory()) { if (recursive) walkJsonl(p, true, out); } else if (e.name.endsWith('.jsonl')) out.push(p);
  }
  return out;
}
...
function streamFile(store, file, o) {
  ...
  for (const line of readLines(file, { start: o.start, end: o.end, maxLine: o.maxLine })) {
    last = line.end;
    if (line.oversized) o.onOversized(line);
    else {
      let rec = null;
      try { rec = JSON.parse(line.text); } catch { if (o.counts) o.counts.malformed++; }
      if (rec) for (const d of o.toDocs(rec, line)) batch.push(d);
    }
    if (batch.length >= BATCH) flush();
    if (o.deadline && Date.now() > o.deadline) { flush(); o.out.pending = true; return; }
  }
  flush();
}
...
function ingestClaudeSource(store, cfg, src, sel, out, opts) {
  let files = walkJsonl(src.path, src.recursive);
  if (sel.mode === 'session') {
  ...
  for (const file of files) {
    ...
    streamFile(store, file, {
      start: offset, end: st.size, maxLine: maxLineOf(cfg), track, rewritten, out, deadline: opts.deadline,
...
function ingestCodexSources(store, cfg, srcs, sel, out, opts) {
  const c = out.codex = { discovered: 0, eligible: 0, otherProject: 0, subagentFiles: 0, unresolved: [], moved: 0, inheritedHeaders: 0,
    malformed: 0, oversized: 0, oversizedMessages: 0, duplicates: 0, incompleteHistory: 0, bytesRead: 0, ...codex.newStats() };
  const track = sel.mode === 'new' || sel.mode === 'all';
  ...
  for (const src of srcs) {
    for (const file of walkJsonl(src.path, src.recursive)) {
  ...
  for (const f of files) {
    ...
    else if (c.unresolved.length < 50) c.unresolved.push({ file: f.state.base, reason: f.state.reason });
  }
  ...
  for (const f of targets) {
    ...
    streamFile(store, f.file, {
      start: offset, end, maxLine, track, rewritten, out, counts: c, deadline: opts.deadline,
      // An oversized record is streamed past, never buffered. If it looks like a message, that is
      // missing conversation and is said so; anything else (a pasted image, a tool dump) is only counted.
      onOversized: (line) => { if (codex.sniff(line.head).isMessage) c.oversizedMessages++; else c.oversized++; },
      toDocs: (rec, line) => {
        c.bytesRead += line.end - line.start;
        if (rec.type === 'session_meta') { if (line.start !== 0) c.inheritedHeaders++; return []; }
        const doc = codex.recordToDoc(rec, ctx, cfg.project, c);
        if (!doc || !inRange(doc.ts, sel)) return [];
        doc.path = f.file; doc.src_offset = line.start;
        return [doc];
      },
      checkpoint: (at) => { f.state.openingSeen = !ctx.openingPending; save(f, at, sha); f.dirty = false; f.prev = { offset: at, sha, size: f.st.size }; },
    });
  }
  ...
  if (track && !out.pending && c.bytesRead > 0 && c.messages === 0 && c.duplicates === 0) {
    out.warnings.push(`codex: ${c.eligible} file(s) of this project were read and NOT ONE conversation message was recognised; the Codex transcript format may have changed (adapter ${codex.ADAPTER})`);
  }
}
...
function ingestMarkdown(store, cfg, sel) {
  ...
  for (const [kind, patterns] of Object.entries(cfg.sources || {})) {
    ...
    const files = [...new Set(asList(patterns).flatMap((p) => globFiles(p)))];
    for (const file of files) {
...
function run(cfg, sel, store, opts = {}) {
  ...
  try {
    ...
    const a = ingestTranscripts(s, cfg, sel, { deadline });
    ...
    return { turns: a.turns, sections: b.sections, skipped: a.skipped + b.skipped, superseded: a.superseded + b.superseded, standing: b.standing,
      pending: a.pending, warnings: a.warnings, codex: a.codex, decisions, oversized: a.oversized || 0, seconds: (Date.now() - t0) / 1000 };
...
function codexReport(c) {
  if (!c) return [];
  const lines = [`codex: ${c.discovered} files found, ${c.eligible} in this project (${c.subagentFiles} subagent files skipped, ${c.otherProject} from other projects, ${c.unresolved.length} unresolved${c.moved ? `, ${c.moved} moved` : ''})`,
  ...
  if (c.oversized) odd.push(`${c.oversized} oversized records skipped`);
...
module.exports = { recordToDoc: claude.recordToDoc, splitSections, parseSelector, ingestTranscripts, ingestMarkdown, run, command, globFiles, codexReport, fingerprint, streamFile };
