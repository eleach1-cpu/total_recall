...
function openStore(file, opts = {}) {
  ...
  const api = {
    ...
    statementTwin: (session, turnId, outcome) => db.prepare(`SELECT id, status FROM docs WHERE kind = 'statement' AND session_id = ? AND outcome = ?
      AND status IN ('active', 'struck') AND ${LAST_EVIDENCE} = ? LIMIT 1`).get(session, outcome, turnId) || null,
    ...
    standing: (limit) => db.prepare(`SELECT * FROM docs WHERE kind = 'statement' AND status = 'active' AND outcome = 'standing' AND superseded_by IS NULL ORDER BY ts DESC LIMIT ?`).all(limit),
    standingCount: () => db.prepare(`SELECT COUNT(*) AS n FROM docs WHERE kind = 'statement' AND status = 'active' AND outcome = 'standing' AND superseded_by IS NULL`).get().n,
...
module.exports = { openStore, migrate, docSha, sha256, SCHEMA_VERSION, SCHEMA_V1 };
