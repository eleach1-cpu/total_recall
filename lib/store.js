...
function openStore(file, opts = {}) {
  ...
  const api = {
    ...
    pendingDecisions: () => db.prepare(`SELECT * FROM docs WHERE kind = 'statement' AND status = 'pending' AND path LIKE 'decide:%' ORDER BY id`).all(),
    ...
    linkDecision: (id, f) => db.prepare(`UPDATE docs SET status = 'active', session_id = ?, source_client = ?, ts = ?, evidence_ids = ?, quote = ?, body = ? WHERE id = ? AND status = 'pending'`)
      .run(f.session_id, f.source_client, f.ts, f.evidence_ids, f.quote, f.body, id).changes,
    setStatus: (id, status) => db.prepare('UPDATE docs SET status = ? WHERE id = ?').run(status, id).changes,
    decisions(f = {}) {
