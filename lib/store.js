...
function openStore(file) {
  ...
  const api = {
    ...
    // nothing automatic (dedupe, reactivation, a retuned prompt) ever brings the row back; the same
    // statement re-derived later has the same sha, so it is ignored and stays struck.
    strike(id, note) {
