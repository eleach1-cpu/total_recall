...
// The owner-decision prompt is for a strong reader going over the backlog: owner decisions only,
// each with what it covers, the proposal it answers, and its doubt kept when there is doubt.
const DECISION_PROMPT = fs.readFileSync(path.join(__dirname, 'decision-prompt.txt'), 'utf8').replace(/\r\n/g, '\n').replace(/\s*$/, '\n');
const BASE = { general: PROMPT, decisions: DECISION_PROMPT };
const PROMPTS = { 'general|claude': { text: PROMPT, sha: PROMPT_SHA } };
function promptFor(client, kind = 'general') {
  const c = client === 'codex' ? 'codex' : 'claude';
  const k = `${kind === 'decisions' ? 'decisions' : 'general'}|${c}`;
  if (!PROMPTS[k]) {
    const base = BASE[k.split('|')[0]];
    const text = c === 'codex' ? base.replace(/CLAUDE/g, 'CODEX').replace(/Claude/g, 'Codex').replace(/claude/g, 'codex') : base;
    PROMPTS[k] = { text, sha: sha256(text) };
  }
  return PROMPTS[k];
}
...
const SYSTEM = 'You answer with one JSON object and nothing else.';
const MAX_OUTPUT_TOKENS = 16000;

// A HARD ceiling on a paid run. A request is sent only when what is already spent PLUS the most
// that request could cost still fits under the cap. "The most" is a bound, not an estimate: a
// token is never shorter than one byte of text, and a reply never passes max_tokens. A call whose
// cost never came back (the line dropped after sending) is counted at that worst case.
function makeMeter(b) {
  if (!(b.cap > 0) || !(b.priceIn > 0) || !(b.priceOut > 0)) throw new Error('a budget needs cap, priceIn and priceOut (dollars, and dollars per million tokens)');
  // `held` is what requests still in flight could cost: with several conversations read at once,
  // each one's worst case is set aside until its real cost comes back, so the cap holds for all of them.
  const m = { cap: b.cap, input: 0, output: 0, calls: 0, assumed: 0, held: 0,
    usd: () => (m.input * b.priceIn + m.output * b.priceOut) / 1e6 + m.assumed,
    worst: (text) => ((Buffer.byteLength(text) + Buffer.byteLength(SYSTEM) + 64) * b.priceIn + (b.maxOutputTokens || MAX_OUTPUT_TOKENS) * b.priceOut) / 1e6,
    add: (u) => { m.calls++; m.input += (u.input_tokens || 0) + 1.25 * (u.cache_creation_input_tokens || 0) + (u.cache_read_input_tokens || 0); m.output += u.output_tokens || 0; },
    assume: (usd) => { m.assumed += usd; } };
  return m;
}

async function callClaude(cfg, model, prompt, env = process.env, opts = null) {
  ...
  const effort = (opts && opts.effort) || (cfg.distill && cfg.distill.effort) || null;
  const betas = useFallbacks ? ['server-side-fallback-2026-07-01'] : [];
  if (auth['anthropic-beta']) betas.push(auth['anthropic-beta']);
  const res = await fetch(url, {
    method: 'POST',
    headers: { ...auth, 'anthropic-version': '2023-06-01', ...(betas.length ? { 'anthropic-beta': betas.join(',') } : {}), 'content-type': 'application/json' },
    body: JSON.stringify({
      // The reply ceiling is also what the spending cap reserves for, so a smaller one is a tighter cap.
      model, max_tokens: (opts && opts.maxOutputTokens) || MAX_OUTPUT_TOKENS, ...(useFallbacks ? { fallbacks: 'default' } : {}), ...(effort ? { output_config: { effort } } : {}),
      system: SYSTEM,
  ...
  if (!res.ok) {
    ...
    // An error status is a request the API did not run, so it is not charged.
    throw Object.assign(new Error(`claude api answered ${res.status} for ${model}${detail ? `: ${detail}` : ''}`), { notBilled: true });
  }
  const j = await res.json();
  if (opts && opts.onUsage && j.usage) opts.onUsage(j.usage); // billed even when the reply is then unusable
...
async function callModel(cfg, opts, prompt) {
  ...
  if (providerOf(cfg, opts) === 'claude') return callClaude(cfg, model, prompt, process.env, opts);
...
async function distillSession(store, cfg, sid, opts) {
  ...
  for (const chunk of chunkTurns(turns, maxChars)) {
    ...
    for (const item of parsed.items) {
      ...
      if (store.statementTwin(sid, cited, s.outcome, s.quote)) continue;
      const r = store.insertDoc({
        ...
        // Identity is the evidence, not the wording: this conversation, this turn, this outcome, these
        // quoted words (one message can hold two decisions).
        sha: sha256(['statement2', sid, cited, s.outcome, quoteKey(s.quote)].join('|')),
...
// ---- the owner-decision pass (a strong reader over the original conversation) ----
const CONTEXT_TURNS = 4;
const SHORT_WORDS = 6;
const quoteKey = decisionQuoteKey;
const DECISION_OUTCOMES = new Set(['approved', 'rejected', 'standing', 'open']);

function renderDecisionSlice(context, turns) {
  const head = context.length ? `CONTEXT (earlier turns, for understanding only):\n${renderChunk(context)}\n` : '';
  return `${head}THE SLICE:\n${renderChunk(turns)}`;
}

// Everything validate() checks (the quote is really in the cited turn, the speaker comes from the
// record), and then what an owner decision additionally needs. A quote that matches proves the
// words were said. It does not prove the reading is right, which is why doubt is carried, not resolved.
function validateDecision(item, turns, context) {
  if (!item || !DECISION_OUTCOMES.has(item.outcome)) return { ok: false, reason: `outcome=${item && item.outcome}` };
  const v = validate(item, turns);
  if (!v.ok) return v;
  const s = v.statement;
  const cited = s.evidence[s.evidence.length - 1];
  if (s.who !== 'owner') return { ok: false, reason: `a decision cited a ${s.who} turn T${cited}` };
  if (/\?\s*$/.test(s.quote)) return { ok: false, reason: `a question is not a decision T${cited}` };
  const said = turns.find((t) => t.id === cited);
  if (said && require('./decide').partOfQuestion(said.body, s.quote)) return { ok: false, reason: `the quoted words are part of a question T${cited}` };
  const all = [...context, ...turns];
  const at = all.findIndex((t) => t.id === cited);
  const wanted = Number(String(item.context_turn ?? '').replace(/^T/i, ''));
  let ctx = wanted ? all.slice(0, at).find((t) => t.id === wanted) : null;
  if (!ctx) ctx = all.slice(0, at).reverse().find((t) => t.role === 'assistant') || null;
  const short = s.quote.split(/\s+/).filter(Boolean).length < SHORT_WORDS;
  if ((s.outcome === 'approved' || s.outcome === 'rejected') && short && !ctx) return { ok: false, reason: `a short reply with no proposal before it T${cited}` };
  let certainty = item.certainty === 'unclear' ? 'unclear' : 'clear';
  // A rule for all future work says so in the owner's own words; otherwise it is held as unclear.
  if (s.outcome === 'standing' && !require('./decide').GENERAL.test(s.quote)) certainty = 'unclear';
  const scope = typeof item.scope === 'string' && item.scope.trim() ? item.scope.trim().slice(0, 120) : 'not stated';
  return { ok: true, statement: { ...s, scope, certainty, evidence: [...(ctx ? [ctx.id] : []), cited] } };
}

async function distillDecisions(store, cfg, sid, opts) {
  const model = modelFor(cfg, opts);
  const extractor = `distill/${model}`;
  const maxChars = (cfg.ollama.chunkTokens || 6000) * 4;
  const out = { sent: 0, skipped: 0, stored: 0, dropped: 0, dropReasons: [], failed: [], replaced: 0, planned: 0, plannedChars: 0 };
  const turns = store.turnsForSession(sid).filter((t) => !t.body.startsWith('(tool-only turn:') && (!t.origin || t.origin === 'direct'));
  const client = (turns[0] && turns[0].source_client) || 'claude';
  const prompt = promptFor(client, 'decisions');
  const tag = `distill:${prompt.sha}`;
  const only = opts.onlyTurns && opts.onlyTurns.length ? new Set(opts.onlyTurns) : null;
  for (const chunk of chunkTurns(turns, maxChars)) {
    if (only && !chunk.turns.some((t) => only.has(t.id))) continue;
    // A finished slice is never paid for twice: the run row carries the model and this prompt.
    const key = { session_id: sid, turn_from: chunk.turn_from, turn_to: chunk.turn_to, model, prompt_sha: prompt.sha };
    if (store.hasRun(key) && !opts.redo) { out.skipped++; continue; }
    const first = turns.indexOf(chunk.turns[0]);
    const context = turns.slice(Math.max(0, first - CONTEXT_TURNS), first);
    const text = prompt.text + renderDecisionSlice(context, chunk.turns);
    if (opts.dry) { out.planned++; out.plannedChars += text.length; continue; }
    // The cap is checked BEFORE sending, against what this request could cost at most.
    const m = opts.meter || null;
    const worst = m ? m.worst(text) : 0;
    if (m && m.usd() + m.held + worst > m.cap) {
      out.capped = true;
      out.capWhy = `stopped BEFORE the $${m.cap} cap: $${m.usd().toFixed(4)} spent${m.held ? `, $${m.held.toFixed(4)} set aside for requests in flight` : ''}, and the next request could cost up to $${worst.toFixed(4)}`;
      break;
    }
    let reply, billed = false;
    const seen = opts.onUsage;
    if (m) m.held += worst;
    try { reply = await callModel(cfg, { ...opts, onUsage: (u) => { billed = true; if (seen) seen(u); } }, text); }
    catch (e) {
      if (m && !billed && !e.notBilled) m.assume(worst); // sent, and what it cost never came back
      recordFailure(out, sid, chunk, e.message, null); continue;
    } finally { if (m) m.held = Math.max(0, m.held - worst); }
    out.sent++;
    const parsed = parseReply(reply);
    if (!parsed) { recordFailure(out, sid, chunk, 'reply was not parseable JSON', reply); continue; }
    const kept = [];
    for (const item of parsed.items) {
      const v = validateDecision(item, chunk.turns, context);
      if (!v.ok) { out.dropped++; if (out.dropReasons.length < 3) out.dropReasons.push(v.reason); continue; }
      kept.push(v.statement);
    }
    // Only now, with a good reply in hand, does a weaker extractor's reading of these turns step
    // aside. Its rows stay in the store. In-session records and owner strikes are never touched.
    const weaker = opts.supersedeWeaker ? store.supersedeWeakerInRange(sid, chunk.turn_from, chunk.turn_to, extractor) : [];
    out.replaced += weaker.length;
    let lines = 0;
    for (const st of kept) {
      const cited = st.evidence[st.evidence.length - 1];
      // Already standing on this turn (recorded in session, or by this same reader), or struck by
      // the owner: the newer model does not overwrite either.
      if (store.statementTwin(sid, cited, st.outcome, st.quote)) continue;
      const r = store.insertDoc({
        project: cfg.project, kind: 'statement', session_id: sid, ts: st.ts, path: tag, source_client: client, adapter: extractor,
        origin: st.certainty === 'unclear' ? 'decision-unclear' : 'decision',
        sha: sha256(['statement3', sid, cited, st.outcome, extractor, quoteKey(st.quote)].join('|')),
        title: `${st.statement} (scope: ${st.scope})`,
        body: `${st.statement} (scope: ${st.scope})\nowner said: ${st.quote}${st.reason ? `\nreason: ${st.reason}` : ''}${st.certainty === 'unclear' ? '\nmeaning or scope UNCLEAR: not a confident approval, not a standing rule' : ''}\nsession ${sid} ${st.ts.slice(0, 10)}`,
        who: 'owner', outcome: st.outcome, evidence_ids: JSON.stringify(st.evidence), quote: st.quote, reason: st.reason,
      });
      if (r.inserted) { out.stored++; lines++; }
    }
    store.pointSuperseded(weaker, extractor);
    store.insertRun({ ...key, ran_at: new Date().toISOString(), lines });
  }
  return out;
}
...
async function run(cfg, sel, opts, store) {
  const own = !store;
  // Money is metered here, from the usage the API itself reports, never from a caller's own count.
  // `opts.meter`: one meter shared by several runs at once (a driver reading conversations in parallel).
  const meter = opts && opts.meter ? opts.meter : opts && opts.budget && providerOf(cfg, opts) === 'claude' ? makeMeter({ ...opts.budget, maxOutputTokens: opts.maxOutputTokens }) : null;
  if (meter) { const seen = opts.onUsage; opts = { ...opts, meter, onUsage: (u) => { meter.add(u); if (seen) seen(u); } }; }
  ...
  try {
    ...
    for (const sid of sessions) {
      ...
      const r = await (opts.kind === 'decisions' ? distillDecisions : distillSession)(s, cfg, sid, opts);
      total.planned = (total.planned || 0) + (r.planned || 0); total.plannedChars = (total.plannedChars || 0) + (r.plannedChars || 0);
      total.replaced = (total.replaced || 0) + (r.replaced || 0);
      if (r.capped) { total.capped = true; total.capWhy = r.capWhy; }
...
async function command(args) {
  ...
  // A dry run sends nothing, so it needs no credentials.
  if (providerOf(cfg, { provider }) === 'claude' && !args.flags.dry && !claudeAuthHeaders(process.env)) {
  ...
  const fl = args.flags;
  const extra = { kind: fl.prompt === 'decisions' ? 'decisions' : 'general', dry: !!fl.dry, supersedeWeaker: !!fl['supersede-weaker'],
    effort: typeof fl.effort === 'string' ? fl.effort : undefined,
    maxOutputTokens: Number(fl['max-output']) > 0 ? Number(fl['max-output']) : undefined,
    onlyTurns: typeof fl.turns === 'string' ? fl.turns.split(',').map((x) => Number(String(x).replace(/^T/i, ''))).filter(Boolean) : undefined };
  const usage = { input: 0, output: 0, calls: 0 };
  const price = { in: Number(fl['price-in']), out: Number(fl['price-out']) };
  const usd = () => (usage.input * price.in + usage.output * price.out) / 1e6;
  if (providerOf(cfg, { provider }) === 'claude' && extra.kind === 'decisions' && !extra.dry) {
    // Real money: never without a ceiling, and never at a price somebody remembered.
    if (!(Number(fl['max-usd']) > 0) || !(price.in > 0) || !(price.out > 0)) {
      process.stderr.write('total_recall distill: a paid decisions run needs --max-usd N and the CURRENT --price-in and --price-out (dollars per million tokens); nothing was sent\n');
      return 1;
    }
    extra.budget = { cap: Number(fl['max-usd']), priceIn: price.in, priceOut: price.out };
  }
  extra.onUsage = (u) => { usage.calls++; usage.input += (u.input_tokens || 0) + (u.cache_creation_input_tokens || 0) + (u.cache_read_input_tokens || 0); usage.output += u.output_tokens || 0; };
  ...
  if (!extra.dry) try { const e = require('./embed'); r.embedded = (await e.run(cfg, { kinds: [...e.EMBED_KINDS, 'turn'] })).embedded; } catch {}
  r.judged = 0;
  // The owner-decision pass draws no model-made "replaced by" links at all: that is the owner's call.
  if (extra.kind !== 'decisions') try { const s = openStore(cfg.store); try { const l = await require('./link').run(cfg, s, { provider, newerThan: r.firstNewId - 1 }); r.linked = l.linked; r.judged = l.judged; } finally { s.close(); } } catch {}
  ...
  if (extra.dry) process.stdout.write(`DRY RUN, nothing sent: ${r.planned} slices would go to ${r.provider} ${r.model}, about ${Math.round(r.plannedChars / 4).toLocaleString()} input tokens at 4 characters each${price.in > 0 ? `, about $${((r.plannedChars / 4) * price.in / 1e6).toFixed(2)} of input at $${price.in} per million (output is extra)` : ''}; ${r.skipped} slices already done\n`);
  if (usage.calls) process.stdout.write(`api: ${usage.calls} calls, ${usage.input.toLocaleString()} input and ${usage.output.toLocaleString()} output tokens${price.in > 0 ? `, $${(r.usd ?? usd()).toFixed(2)} at the prices given` : ''}; ${r.replaced || 0} weaker extractions stepped aside\n`);
  // Said even when not one request went out: a cap too small for the first slice is still a stop.
  if (r.capped) process.stdout.write(`CAP REACHED, ${r.capWhy}; the unfinished slices wait for the next run\n`);
  if (r.failed.length) {
...
module.exports = { makeMeter, PROMPT, PROMPT_SHA, promptFor, speakerOf, distillDecisions, validateDecision, renderDecisionSlice, chunkTurns, renderChunk, parseReply, validate, fuzzyQuote, distillSession, run, command,
