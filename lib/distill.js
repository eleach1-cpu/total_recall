...
function validate(item, turns) {
  ...
  const who = turns[idx].role === 'user' ? 'owner' : 'claude';
  // Only the owner can set a standing rule; a Claude line tagged standing is a mislabel.
  if (item.outcome === 'standing' && who !== 'owner') return { ok: false, reason: `standing cited a claude turn T${turnId}` };
  // Only the owner approves; Claude's "Agreed" is not an approval of anything.
  if (item.outcome === 'approved' && who !== 'owner') return { ok: false, reason: `approved cited a claude turn T${turnId}` };
...
async function distillSession(store, cfg, sid, opts) {
  ...
  const turns = store.turnsForSession(sid).filter((t) => !t.body.startsWith('(tool-only turn:'));
  for (const chunk of chunkTurns(turns, maxChars)) {
    ...
    let reply;
    try { reply = await callModel(cfg, opts, PROMPT + renderChunk(chunk.turns)); }
    ...
    for (const item of parsed.items) {
      ...
      const r = store.insertDoc({
        ...
        project: cfg.project, kind: 'statement', session_id: sid, ts: s.ts, path: `distill:${PROMPT_SHA}`,
...
async function run(cfg, sel, opts, store) {
  ...
  const t0 = Date.now();
  try {
    // Statements written by an older prompt leave the active set; the new prompt re-derives them
    // (their run rows carry the old prompt sha, so every chunk is sent again).
    const retired = s.supersedeStatementsExcept(`distill:${PROMPT_SHA}`);
    if (retired && opts.onRetire) opts.onRetire(retired);
    s.dedupeStatements();
    let sessions;
...
async function command(args) {
  ...
  try { r.embedded = (await require('./embed').run(cfg, {})).embedded; } catch {}
  try { const s = openStore(cfg.store); try { r.linked = (await require('./link').run(cfg, s, { provider })).linked; } finally { s.close(); } } catch {}
  process.stdout.write(`distilled ${r.sessions} sessions: ${r.sent} chunks sent, ${r.skipped} already done, ${r.stored} statements stored, ${r.dropped} dropped by validation${r.dropReasons.length ? ` (${r.dropReasons.join('; ')})` : ''}, ${r.embedded} embedded, ${r.linked} superseded links, ${r.seconds.toFixed(1)} seconds, ${r.provider} ${r.model}\n`);
  if (r.failed.length) {
...
module.exports = { PROMPT, PROMPT_SHA, chunkTurns, renderChunk, parseReply, validate, fuzzyQuote, distillSession, run, command,
