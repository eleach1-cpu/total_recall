...
function makeMeter(b) {
  ...
  const m = { cap: b.cap, input: 0, output: 0, calls: 0, assumed: 0,
    usd: () => (m.input * b.priceIn + m.output * b.priceOut) / 1e6 + m.assumed,
    worst: (text) => ((Buffer.byteLength(text) + Buffer.byteLength(SYSTEM) + 64) * b.priceIn + MAX_OUTPUT_TOKENS * b.priceOut) / 1e6,
...
async function callClaude(cfg, model, prompt, env = process.env, opts = null) {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({
      model, max_tokens: MAX_OUTPUT_TOKENS, ...(useFallbacks ? { fallbacks: 'default' } : {}), ...(effort ? { output_config: { effort } } : {}),
      system: SYSTEM,
...
async function distillDecisions(store, cfg, sid, opts) {
  ...
  for (const chunk of chunkTurns(turns, maxChars)) {
    ...
    if (m && m.usd() + worst > m.cap) {
      out.capped = true;
      out.capWhy = `stopped BEFORE the $${m.cap} cap: $${m.usd().toFixed(4)} spent, and the next request could cost up to $${worst.toFixed(4)}`;
      break;
    }
    const callsBefore = m ? m.calls : 0;
    let reply;
    try { reply = await callModel(cfg, opts, text); }
    catch (e) {
      if (m && m.calls === callsBefore && !e.notBilled) m.assume(worst); // sent, and what it cost never came back
...
async function run(cfg, sel, opts, store) {
  ...
  const meter = opts && opts.budget && providerOf(cfg, opts) === 'claude' ? makeMeter(opts.budget) : null;
...
module.exports = { PROMPT, PROMPT_SHA, promptFor, speakerOf, distillDecisions, validateDecision, renderDecisionSlice, chunkTurns, renderChunk, parseReply, validate, fuzzyQuote, distillSession, run, command,
