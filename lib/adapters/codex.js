...
function newStats() {
  return { messages: 0, injectedBlocks: 0, injectedOnly: 0, unknownPhase: 0, unverifiedOpenings: 0, badTimestamp: 0, inherited: 0, eventCopies: 0, agentMessages: 0, ignored: {} };
}
...
function recordToDoc(rec, ctx, project, stats) {
  ...
  if (rec.type === 'event_msg') {
    ...
    if (item && (item.type === 'UserMessage' || item.type === 'AgentMessage')) { stats.eventCopies++; return null; }
  ...
  let text;
  if (p.role === 'user') text = userText(p.content, stats);
  else {
...
module.exports = { ADAPTER, metaOf, segmentOf, recordToDoc, ownerTextOf, newStats, sniff };
