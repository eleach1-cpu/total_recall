...
//   PostToolUse   this server's recall_search/recall_recall : successful retrieval opens this task's gate
//
...
// server can serve several tasks). Refusal is a structured PreToolUse deny on stdout.
//
...
// The structured note retrieval attaches to its result, wherever the host's envelope puts it.
...
function handle(payload, io = { out: process.stdout, err: process.stderr }) {
  ...
  if (event === 'PostToolUse') {
    const completedTool = tool.match(/(?:^|__)(recall_search|recall_recall)$/)?.[1];
    if (!completedTool || !sid) return 0;
    ...
    if (done && done.tool === completedTool && done.ok === true && done.project === cfg.project) gate.ack(cfg.project, sid, 'codex');
    return 0;
  }
  if (event === 'PreToolUse') {
    let r = { ok: true };
    ...
    if (r.ok) return 0;
    // Use Codex's structured refusal. A live desktop test showed an exit-code-only
    // refusal reaching the hook without stopping the nested apply_patch call.
    io.out.write(JSON.stringify({ hookSpecificOutput: {
