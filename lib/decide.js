...
// Owner, 2026-09-18: questions are not decisions. A quote cut short so that it no longer shows its
// question mark is still part of a question: look at where the owner's sentence actually ends.
// (A full stop inside a file name, "config.toml", does not end a sentence.)
function partOfQuestion(body, quote) {
  const q = String(quote || '').trim();
  if (!q) return false;
  const esc = (w) => w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const at = new RegExp(q.split(/\s+/).map(esc).join('\\s+'), 'i').exec(String(body || ''));
  if (!at) return false; // a fuzzy-matched quote has no place to look from
  const end = /[!?\n]|\.(?=\s|$)/.exec(String(body).slice(at.index + at[0].length));
  return !!end && end[0] === '?';
}
...
const UNVERIFIED_TEXT = 'UNVERIFIED: no matching owner evidence was linked after ingest; this is not a verified decision';
const bodyOf = (r, linked, status = 'pending') => [`${r.statement} (scope: ${r.scope})`, `owner said: ${r.quote}`, r.reason ? `reason: ${r.reason}` : null,
  r.certainty === 'unclear' ? 'meaning or scope UNCLEAR: not a confident approval, not a standing rule' : null,
  linked ? `session ${linked.session} ${linked.ts.slice(0, 10)}` : status === 'unverified' ? UNVERIFIED_TEXT : 'PENDING: the owner\'s words have not been found in the ingested conversation yet'].filter(Boolean).join('\n');

// Old records can have a stale PENDING sentence even though their status is
// unverified. Correct the presentation without mutating a store during Read.
function displayBody(d) {
  return d.kind === 'statement' && String(d.path || '').startsWith('decide:') && d.status === 'unverified'
    ? d.body.replace(/^PENDING:.*$/m, UNVERIFIED_TEXT) : d.body;
}
function resultText(r) {
  if (r.status === 'active') return `linked to ${(r.evidence || []).map(id => `T${id}`).join(', ')}`;
  if (r.status === 'unverified') return UNVERIFIED_TEXT;
  return `${String(r.status).toUpperCase()} (${r.why || 'not linked yet'}); the next ingest checks it again`;
}
...
function findEvidence(store, rec, quote, contextQuote) {
  ...
  for (const h of hits) {
    ...
    if (h.turn.adapter === 'codex/2:question-answer') {
      const ctx = earlier.find(t => t.sha === sha256(`${h.turn.sha}|question`));
      if (ctx && (!contextQuote || holds(ctx.body, contextQuote))) candidates.push({ ...h, context: ctx });
      continue;
    }
    if (contextQuote) {
...
function tryLink(store, doc, meta, resolved = null) {
  ...
  if (partOfQuestion(ev.turn.body, ev.quote)) return { linked: false, why: 'those words are part of a question the owner asked, and a question is not a decision' };
...
function record(store, cfg, input) {
  ...
  if (!ins.inserted) return { id: doc.id, duplicate: true, status: doc.status, evidence: JSON.parse(doc.evidence_ids || '[]'), note: r.note };
...
function linkPending(store) {
  ...
  for (const doc of store.pendingDecisions()) {
    ...
    if (result.ambiguous) { out[doc.status === 'unverified' ? 'unverified' : 'waiting']++; continue; }
    ...
    if (doc.status === 'unverified' || store.hasTurnAfter(doc.source_client, new Date(Date.parse(m.recorded_at) + 10 * 60000).toISOString())) {
      store.setStatus(doc.id, 'unverified', bodyOf(meta, null, 'unverified')); out.unverified++;
    }
    else out.waiting++;
  }
  return out;
...
function command(args) {
  ...
  try {
    ...
    const where = resultText(r);
    process.stdout.write(`${r.duplicate ? 'already recorded' : 'recorded'} #${r.id}: ${where}${r.note ? `\nnote: ${r.note}` : ''}\n`);
    return 0;
  } finally { store.close(); }
}

module.exports = { record, linkPending, listing, check, containsPhrase, partOfQuestion, command, GENERAL, displayBody, resultText };
